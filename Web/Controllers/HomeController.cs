...
public class HomeController : Controller
{
    ...
    public async Task<IActionResult> Index()
    {
        try
        {
            ...
            var meals = await _mealService.GetMealsByUserId(sessionUser.UserId, PAGE_SIZE, 1);
        ...
        {
            _logger.LogError(e, "An error occurred while loading the home page.");
            throw;
        }
    }
    ...
    [HttpGet]
    public async Task<IActionResult> LoadMoreMealHistory(int pageIndex)
    {
        var sessionUser = await _accessSessionHelper.GetUserContextAsync();
        var meals = await _mealService.GetMealsByUserId(sessionUser.UserId, PAGE_SIZE, pageIndex);
